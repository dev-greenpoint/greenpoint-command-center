const express = require('express');
const router = express.Router();
const { query } = require('../db/database');

router.get('/', async (req, res) => {
  res.json(await query('SELECT * FROM team_members ORDER BY name ASC'));
});

router.post('/', async (req, res) => {
  const { name, role, email } = req.body;
  if (!name) return res.status(400).json({ error: 'Name is required' });
  const [{ id }] = await query(
    'INSERT INTO team_members (name, role, email) VALUES (?, ?, ?) RETURNING id',
    [name, role || null, email || null]
  );
  res.status(201).json({ id });
});

router.patch('/:id', async (req, res) => {
  const existing = await query('SELECT * FROM team_members WHERE id=?', [req.params.id]);
  if (!existing.length) return res.status(404).json({ error: 'Not found' });
  const cur = existing[0];
  const b = req.body;
  await query(
    'UPDATE team_members SET name=?, role=?, email=? WHERE id=?',
    [
      'name' in b ? b.name : cur.name,
      'role' in b ? (b.role || null) : cur.role,
      'email' in b ? (b.email || null) : cur.email,
      req.params.id
    ]
  );
  res.json({ ok: true });
});

router.delete('/:id', async (req, res) => {
  await query('DELETE FROM team_members WHERE id=?', [req.params.id]);
  res.json({ ok: true });
});

// Names-only list — used by sidebar and assignee selectors
router.get('/names', async (req, res) => {
  const rows = await query('SELECT name FROM team_members ORDER BY name ASC');
  res.json(rows.map(r => r.name));
});

module.exports = router;
