...
router.get('/:clientId', async (req, res) => {
  ...
  const client = rows(db.exec('SELECT slack_channel, drive_folder_url FROM clients WHERE id=?', [req.params.clientId]))[0];
...
router.post('/:clientId/slack', async (req, res) => {
  ...
  db.run('UPDATE clients SET slack_channel=? WHERE id=?', [slack_channel, req.params.clientId]);
...
router.post('/:clientId/drive', async (req, res) => {
  ...
  db.run('UPDATE clients SET drive_folder_url=? WHERE id=?', [drive_folder_url, req.params.clientId]);
  db.run(
